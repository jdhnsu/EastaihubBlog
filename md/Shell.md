...
- 如果你使用的是`windows` ，可以使用 `cmd` 启动一个终端,但还本文使用的命令属于`Unix`风格，`cmd`可能无法执行某些命令。
所以建议在Windows 中建议使用 `WSl` 或者 `pwsh` 来跟进课程。
   - [wsl](https://learn.microsoft.com/zh-cn/windows/wsl/install) 是 Windows 10 的一个功能，允许您使用 Linux 的命令行界面，本质上是一个虚拟机，但是这一直接调用部分硬件，如 CPU、内存、硬盘、显卡等等，使用 WSL 来做一些开发任务也十分方便。
   - [pwsh](https://learn.microsoft.com/zh-cn/powershell/scripting/install/installing-powershell-on-windows?view=powershell-7.5) 是 PowerShell 的一个实现，PowerShell 是一个开源的跨平台 shell。因为`pwsh`是跨平台的，所以在 Windows 和 Linux 中都是可用的，，它允许您使用 .NET 框架，因此您可以使用 C# 或 PowerShell 来编写脚本，并且对于一些`bash`命令可以直接映射执行。

- 对于`MacOS` 中，可以使用 `terminal`,应为`MacOS` 是一个 Unix 系统，所以它也支持本课程的大多数 shell 命令。
- 对于`Linux` 中，可以使用 `bash` 启动一个终端，本课程使用的就是`linux`系统。
...
missing:~$ man ls
```
这时候你的屏幕差不多已经满了，你可使用`clear`或 `Ctrl+L` 来清屏
